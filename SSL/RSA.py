...
class RSA():
    ...
    def encrypt(self, public_key, plaintext='', ):
        blocks = self.get_plaintext(plaintext)  # 获取明文分块并转码为数字
        out = []
        for i in range(len(blocks)):
            m = blocks[i]
            c = self.quick_pow_mod(m, public_key[0], public_key[1])  # 加密
    ...
    def get_plaintext(self, text=''):  # 获取明文 转换为ascii
        # print('请输入明文：')
        # text = input() # 取消掉这行注释
        self.plaintext = text
...
rsa = RSA()
# cipher = rsa.encrypt(rsa.public_key, 'abcdefghijklmnopqrstuvwxyz')
# plain = rsa.decrypt(rsa.private_key, cipher)
# print(plain)
#
# Hash = hashlib.md5(rsa.plaintext.encode('utf-8')).hexdigest()
# print(Hash)
# Hash = rsa.encrypt(rsa.public_key, Hash)
# print(Hash)
# Hash = rsa.decrypt(rsa.private_key, Hash)
# print(Hash)

# public_key = 1000111000
# crypto_type = 'RSA'
# msg = f"{crypto_type},{public_key}".encode("utf-8")
# hash = hashlib.md5(msg).hexdigest()
# signiture = rsa.encrypt(rsa.private_key, hash)
# certificates = f"{crypto_type},{public_key},{signiture}".encode("utf-8")
#
# type, key, sig = certificates.decode("utf-8").split(',')
# sig = int(sig.strip('[').strip(']'))
# m = rsa.quick_pow_mod(sig, rsa.public_key[0], rsa.public_key[1])  # 解密
# m = rsa.decode(m)
#
#
# print(m.strip('0'))
